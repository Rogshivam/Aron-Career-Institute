...
const sendOTP = (email, title, otp) => {
  return mg.messages.create(DOMAIN, {
    from: `Aron Career Institute <${process.env.EMAIL_USER}>`,
    to: [email],
    subject: `Aron Career Institute ${title} OTP`,
...
const sendMessage = (email, name, message) => {
  return mg.messages.create(DOMAIN, {
    from: `Aron Career Institute <${process.env.EMAIL_USER}>`,
    to: [email],
    subject: 'Aron Career Institute Important Alert',
    text: `Dear ${name},\n\n${message}`,
    html: `
      <p>Dear ${name},</p>
      <p>${message}</p>
      <br />
      <strong>Best Regards from Aron Career Institute!</strong>
    `
  });
};
